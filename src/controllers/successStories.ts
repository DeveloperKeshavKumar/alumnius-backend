...
export const createSuccessStory = async (req: Request, res: Response): Promise<void> => {
   try {
      const { title, story, alumniId } = req.body;
...
export const getSuccessStoryById = async (req: Request, res: Response): Promise<void> => {
   try {
      ...
      const [story] = await db.select().from(successStoriesTable).where(eq(successStoriesTable.id, id)).limit(1);
...
export const updateSuccessStory = async (req: Request, res: Response): Promise<void> => {
   try {
      ...
      const [updatedStory] = await db.update(successStoriesTable).set(updates).where(eq(successStoriesTable.id, id)).returning();
